...
\begin{itemize} %\setlength\itemsep{-\parsep}   %
  \item[\normalsize{\ding{110}}] 思考如何对论文进行修改拓展，然后转投期刊  \vspace{-3 mm}\\
  % \subitem {\footnotesize{\ding{108}}} 
  \item[\normalsize{\ding{110}}] 调研世界模型在无人机视觉语言导航领域的发展  \vspace{-3 mm}
\end{itemize}                      % --> To Here
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{enumerate}\setlength\itemsep{-\parsep}
  ...
  \item \Large\textbf{对论文进行修改拓展，转投期刊} 
  
  % \large\textbf{数据收集：} 
  \normalsize{
      分析论文中目前的方法问题，思考一下论文的 V2 版本，然后
      寻找一个合适的期刊，加上 AerialVLN 数据集的结果，转投。
  }
  ...
  \item \Large\textbf{调研世界模型在无人机视觉语言导航领域的发展}
  
  \normalsize{
    世界模型的核心定位是从“反应式执行” 转向 “预见式规划”。
    \begin{itemize}
      \item 将抽象的自然语言指令显式地转化为未来第一人称视角的视觉序列；
      \item 提供运动先验与空间预见性，在复杂城市（严重遮挡、剧烈视角切换）场景中，
      世界模型通过生成未来画面，让智能体提前“看到”转弯处的环境，避免纯历史观测策略的“短视”。
      \item 利用想象未来的动态特征来优化决策，
      而世界模型则根据规划轨迹调整生成内容，实现闭式协同训练。
    \end{itemize} 

    同时也有些问题需要改进：
    \begin{itemize}
      \item 巨大的推理延迟与计算负担（核心痛点）；
      \item 像素级生成质量与时空一致性不足；
      \item 模拟到现实（Sim-to-Real）的迁移鲁棒性；
    \end{itemize} 
  }
...
\begin{itemize}
  \item[\normalsize{\ding{110}}] 
    调研一下可扩展的期刊，同时思考如何将论文的方法问题进行改进 \vspace{-3 mm}\\
  \item[\normalsize{\ding{110}}] 
    阅读论文，思考无人机视觉对话导航方向的问题。
    \begin{itemize}
      \item 研究改进提问时机
      \item 研究模型提问问题的结构
      \item 研究拓扑图或者其他数据结构的记忆机制
    \end{itemize}
    % \vspace{-3 mm}\\

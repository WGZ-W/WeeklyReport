...
\begin{enumerate}\setlength\itemsep{-\parsep}
  ...
  \item \Large\textbf{ Fixed bugs in the code} 
  ...
  \normalsize{
    ...
    \begin{itemize}
      \item  NCCL communication timeout
      
      
      Cause analysis: data loading is imbalanced. 
      Each trajectory has a different length, and trajectory lengths within a batch are severely uneven. 
      In addition, the data distributed to each GPU by DDP is random. 
      Therefore, the solution is to first traverse the dataset, 
      sort samples by length, and then assign them to each rank 
      using a greedy strategy so that the total lengths are similar. 
      After that, the data is padded to the longest trajectory length within each batch.
      
      \item  CUDA kernel assertion failure 
      
      The main issues include: 
      incorrect data preprocessing, 
      where invalid actions were mapped to -1 
      but not handled properly; 
      abnormal batch processing in distributed training; 
      inconsistent action-space size definitions; 
      and invalid labels in the dataset. 
      First, I checked the data preprocessing code and found no problem. 
      Then I checked the code defining the action-space size and found no problem either. 
      Some assertions were added to the distributed training batch-processing 
      code to catch exceptions during training. 
      Finally, I checked whether invalid labels existed in the dataset. 
      The results showed that they did exist, but the number was small, so they can be removed directly.
